 ...
 * Version 1.0.1
 * Added ability to drink water directly from a water block
...
public class DrinkOrDie extends JavaPlugin {
	...
	public void onEnable() {
		...
		ConfigurationSerialization.registerClass(SurvivalPlayer.class, "survivalPlayer");
		...
		try {
			thirstPeriod = Long.parseLong(this.getConfig().getString("thirstPeriod"));
		} catch (NumberFormatException e) {
			this.getLogger().log(Level.SEVERE, "thirstLevel in config.yml must be set to an integer value, eg. 200. Loading a default value.");
		}
		...
		SpoutManager.getFileManager().addToPreLoginCache(this, "https://dl.dropbox.com/u/44954718/mywaterpouch2.png");
		SpoutManager.getFileManager().addToPreLoginCache(this, "https://dl.dropbox.com/u/44954718/myWaterPouchWater5.png");		
		SpoutManager.getFileManager().addToPreLoginCache(this, "https://dl.dropbox.com/u/44954718/myWaterDrop.png");		
		SpoutManager.getFileManager().addToPreLoginCache(this, "https://dl.dropbox.com/u/44954718/myWaterDropDepleted.png");		
		...
		SpoutShapedRecipe leatherWaterPouchRecipe = new SpoutShapedRecipe(leatherWaterStack);
		...
		SpoutManager.getMaterialManager().registerSpoutRecipe(leatherWaterPouchRecipe);
		
		LeatherPouch leatherPouch = new LeatherPouch(this,"", "");
		SpoutItemStack leatherPouchStack = new SpoutItemStack(leatherPouch, 1);
		SpoutShapedRecipe leatherPouchRecipe = new SpoutShapedRecipe(leatherPouchStack);
		leatherPouchRecipe.shape(new String[] { " L ", "L L", " L " });
		leatherPouchRecipe.setIngredient('L', MaterialData.leather);
		SpoutManager.getMaterialManager().registerSpoutRecipe(leatherPouchRecipe);
		if(modEnabled) {
		...
		if (cmd.getName().equalsIgnoreCase("thirstperiod")) {
			if(args.length == 1) {
				try {
					...
					sender.sendMessage("The thirst period has been set to: " + thirstPeriod + ". \nPlease reload the server to apply changes.");
					return true;
				} catch (NumberFormatException e) {
